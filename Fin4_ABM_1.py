...
simulation_parameters ={
    'T': 1, # range(int(config['general']['time_steps'])),
    'N': 1,
    'M': {}
}
...
def random_claim_of_tokens(params, step, sL, s):
    ...
    for pt in s['PATs']:
        pats_all[pt['design']][pt['purpose']].append(pt)
    ...
    for compliant in agents_all["compliant"]:
        total_pats = sum(d[compliant['claimer_PAT_intention']] for d in pats_all['careful'].values() if d)  # https://www.geeksforgeeks.org/python-sum-values-for-each-key-in-nested-dictionary/
        print("total pats")
        print(total_pats)
        claim(compliant, total_pats, 'add_activity', s)
        
    for compliant in agents_all["opportunistic"]:
        total_pats = sum(d[compliant['claimer_PAT_intention']] for d in pats_all['careful'].values() if d)  # https://www.geeksforgeeks.org/python-sum-values-for-each-key-in-nested-dictionary/
        claim(compliant, total_pats, 'add_activity', s)
...
def random_claim_of_tokens_old(params, step, sL, s):
    agents = s['agents']
    pats = s['PATs']
    ...
    #print("-------------------------------------------------------compliant, opport, cheat:") 
    print(compliant_agents_claiming)
    print(opportunistic_agents_claiming)
    #g = opportunistic_agents_claiming
    #print(cheater_agents_claiming)
    result = {'update_agents': {'update': compliant_agents_claiming},
            'update_agents': {'update': opportunistic_agents_claiming},
            'update_agents': {'update': cheater_agents_claiming}}
...
"""### State update blocks"""
print(config['PAT agents']['custom_bootstrapping'] == 'True')
if config['PAT agents']['custom_bootstrapping'] == 'True':
